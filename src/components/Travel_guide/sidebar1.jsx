...
export default function Sidebar(props) {
    ...
    const navList = [
        { icon: <RectangleGroupIcon className="h-5 w-5" />, name: "Dashboard", link: "/guide_dashboard" },
        {
    ...
    return (
        <Card
            className={`${isSidebarOpen ? 'w-[16.25rem]' : 'w-[6.5rem]'
        ...
        >
            ...
            <div className="">
                <Typography variant="h5" color="white">
                    <img
                        src={isSidebarOpen ? '../public/Travel_guide/logo.png' : '../public/Travel_guide/minilogo.svg'}
                        alt="Logo"
            ...
            <button onClick={handleSidebarToggle} className='z-10 '>
                <ChevronLeftIcon
                    className={`h-[24px] w-[24px] bg-[#2AB57D] rounded-full z-10 gap-3p-1 absolute -right-[0.7rem] ${isIconRotated ? 'rotate-180 transition-all duration-300 ease-in-out' : 'transition-all duration-300 ease-in-out'
                        }`}
                />
            </button>
                    ...
                    navList.map((item, key) => (
                        <React.Fragment key={key}>
                            <Link to={item.link} className='w-[100%] !style:none' key={key}>
                                <ListItem className={`gap-[2rem] hover:bg-[#FFFFFF] hover:bg-opacity-30 active:bg-[#2AB57D] focus:bg-[#2AB57D] active:text-white focus:text-white ${active === item.name ? "active" : ""}`} onClick={item.name === 'My Trips' ? handleSubSidebarToggle : undefined}>
                                    {item.icon}
                                ...
                                <div className="ml-[2.5rem] mt-1 space-y-1">
                                    {item.subItems.map((subItem, subKey) => (
                                        <Link to={subItem.link} className="w-[100%] !style:none" key={subKey}>
                                            <ListItem
                                                className={`gap-[2rem] hover:bg-[#FFFFFF] hover:bg-opacity-30 active:bg-[#2AB57D] focus:bg-[#2AB57D] active:text-white focus:text-white ${active === subItem.name ? 'active' : ''
                                                    }`}
                                            >
                                                {subItem.icon}
