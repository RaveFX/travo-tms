 ...
 const styles = {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const ActivitiesResult = await axios.get(`http://localhost:8080/api/v1/trip/activityList/${id}/${day}`);
        ...
        const filteredActivities = ActivitiesResult.data.filter((activity) => {
          const nearbyAttractions = attractionsResult.data.filter((attraction) => {
            const distance = calculateDistance(attraction.latitude, attraction.longitude, activity.latitude, activity.longitude);
...
export default CloserActivities